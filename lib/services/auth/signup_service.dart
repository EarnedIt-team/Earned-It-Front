import 'package:dio/dio.dart';
import 'package:earned_it/main.dart';
import 'package:earned_it/models/api_response.dart';
import 'package:earned_it/services/rest_client.dart';
import 'package:flutter_riverpod/flutter_riverpod.dart';

final signUpServiceProvider = Provider<SignUpService>((ref) {
  return SignUpService(restClient);
});

class SignUpService {
  final RestClient _restClient;

  SignUpService(this._restClient);

  /// 이메일 인증 코드를 서버에 요청합니다.
  Future<void> sendEmailAuthCode(String email) async {
    try {
      final ApiResponse response = await _restClient.sendEmail(email);
      // 통신은 성공했지만, 처리가 되지 않았을 때
      if (response.code != "SUCCESS") {
        throw Exception(response.message);
      }
      // 400 에러 등
    } on DioException catch (e) {
      throw Exception(e.response!.data["message"]);
    } catch (e) {
      // DioException이 아닌 다른 예외 발생 시 (네트워크 연결 끊김 등)
      throw Exception("서버에서 에러가 발생했습니다.");
    }
  }

  /// 인증 코드가 유효한지 서버에 요청합니다.
  Future<void> verifyEmail(String email, String code) async {
    try {
      final Map<String, String> requestBody = <String, String>{
        "email": email,
        "token": code,
      };

      final ApiResponse response = await _restClient.verifyEmail(requestBody);
      // 통신은 성공했지만, 처리가 되지 않았을 때
      if (response.code != "SUCCESS") {
        throw Exception(response.message);
      }
      // 400 에러 등
    } on DioException catch (e) {
      throw Exception(e.response!.data["message"]);
    } catch (e) {
      // DioException이 아닌 다른 예외 발생 시 (네트워크 연결 끊김 등)
      throw Exception("서버에서 에러가 발생했습니다.");
    }
  }
}
