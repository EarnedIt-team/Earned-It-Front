...
class UserService {
  ...
  /// 프로필 정보를 서버에서 불러옵니다.
  Future<ApiResponse> loadProfile(String accessToken) async {
    try {
      String token = "Bearer $accessToken";

      final ApiResponse response = await _restClient.loadProfile(token);
      // 통신은 성공했지만, 처리가 되지 않았을 때
      if (response.code != "SUCCESS") {
        throw Exception(response.message);
      }
  ...
  /// 다른 사용자의 프로필 정보를 서버에서 불러옵니다.
  Future<ApiResponse> loadOtherProfile(String accessToken, int userId) async {
    try {
      String token = "Bearer $accessToken";

      final ApiResponse response = await _restClient.loadOtherProfile(
        token,
        userId,
      );
      // 통신은 성공했지만, 처리가 되지 않았을 때
